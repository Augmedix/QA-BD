...
def nrt_api(serverurl,streamingtype,mediatype,filepath,emailid,password,authurl,patient_creation_url,patient_name):
    ...
    docid = decoded["uid"]
    item_list.append(token)
    item_list.append(docid)
    ...
    if response[0].ok == False:
        ...
        print("Patient created successfully :", response[0].text)
        patient_info_parse = json.loads(response[0].text)
        noteid = patient_info_parse["noteId"]
        item_list.append(noteid)
        visit_start_time = response[1]
        if noteid == 0:
    ...
    return success, visit_start_time, item_list
