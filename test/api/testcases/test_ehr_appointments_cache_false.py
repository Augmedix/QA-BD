...
class TestEHRAppointmentsCacheFalse(BaseTest):
    ...
    def test_appointments_loaded_properly_for_a_specific_date_range_with_valid_provider(self):
        ...
        request_path = f'lynx/appointments?cache.invalidateCache=false&endDate={end_date}&startDate={start_date}&doctorId={doctor_id}'
        ...
        with allure.step('Proper datalist, status_code and reason should be returned for all appointments'):
            ...
            for i in range(len(data_list)):
                assert len(data_list[i]['uuid']) == 36
                assert len(data_list[i]['appointmentId']) == 7
                assert re.fullmatch(r'\d+', data_list[i]['patientId'])
                assert len(data_list[i]['patientFirstName']) > 0
                assert re.fullmatch(r'\d{4}-\d{2}-\d{2}', data_list[i]['patientBirthDate'])
                assert re.fullmatch(r'[mfoMFO]', data_list[i]['patientGender'])
                assert re.fullmatch(r'\d{2}:\d{2} [AP]M', data_list[i]['appointmentStartTime'])
    ...
    def test_appointments_loaded_properly_for_a_specific_date_with_valid_provider(self):
        doctor_id = pytest.configs.get_config('ehr_lynx_enabled_rt_provider_id')
        request_path = f'lynx/appointments?cache.invalidateCache=false&startDate={end_date}&doctorId={doctor_id}'
        response = RequestHandler.get_api_response(user_name=pytest.configs.get_config('ehr_lynx_enabled_rt_provider'),
                                                   password=pytest.configs.get_config('all_provider_password'),
                                                   request_path=request_path)
        with allure.step('Proper datalist, status_code and reason should be returned for all appointments'):
    ...
    def test_appointments_should_not_load_without_provider_id_returns_proper_msg(self):
        request_path = f'lynx/appointments?cache.invalidateCache=false&endDate={end_date}&startDate={start_date}'
        response = RequestHandler.get_api_response(user_name=pytest.configs.get_config('ehr_lynx_enabled_rt_provider'),
                                                   password=pytest.configs.get_config('all_provider_password'),
                                                   request_path=request_path)
        with allure.step('Proper datalist, status_code and reason should be returned for all appointments'):
    ...
    def test_doctor_should_not_get_ehr_appointment_without_start_date_as_params(self):
        doctor_id = pytest.configs.get_config('ehr_lynx_enabled_rt_provider_id')
        request_path = f'lynx/appointments?cache.invalidateCache=false&doctorId={doctor_id}&endDate={end_date}'
        response = RequestHandler.get_api_response(user_name=pytest.configs.get_config('ehr_lynx_enabled_rt_provider'),
                                                   password=pytest.configs.get_config('all_provider_password'),
                                                   request_path=request_path)
    ...
    def test_doctor_should_not_get_ehr_appointment_without_doctor_id_and_start_date_as_params(self):
        request_path = f'lynx/appointments?cache.invalidateCache=false&endDate={end_date}'
        response = RequestHandler.get_api_response(user_name=pytest.configs.get_config('ehr_lynx_enabled_rt_provider'),
                                                   password=pytest.configs.get_config('all_provider_password'),
                                                   request_path=request_path)
        with allure.step('Proper massage, status_code and reason should be returned'):
    ...
    def test_doctor_should_not_get_ehr_appointment_data_using_start_date_after_end_date_as_params(self):
        doctor_id = pytest.configs.get_config('ehr_lynx_enabled_rt_provider_id')
        request_path = f'lynx/appointments?cache.invalidateCache=false&doctorId={doctor_id}&startDate={end_date}&endDate={start_date}'
        response = RequestHandler.get_api_response(user_name=pytest.configs.get_config('ehr_lynx_enabled_rt_provider'),
                                                   password=pytest.configs.get_config('all_provider_password'),
                                                   request_path=request_path)
        with allure.step('Proper massage, status_code and reason should be returned'):
    ...
    def test_doctor_should_not_get_ehr_appointment_data_for_valid_doctor_id_with_token_from_another_ehr_lynx_enabled_user(self):
        doctor_id = pytest.configs.get_config('ehr_lynx_enabled_rt_provider_id')
        request_path = f'lynx/appointments?cache.invalidateCache=false&doctorId={doctor_id}&startDate={start_date}&endDate={end_date}'
    ...
    @allure.severity(allure.severity_level.CRITICAL)
    @pytest.mark.security
    def test_security_doctor_should_not_get_ehr_appointment_data_with_algorithm_set_as_none_token(self):
        doctor_id = pytest.configs.get_config('ehr_lynx_enabled_rt_provider_id')
        request_path = f'lynx/appointments?cache.invalidateCache=false&doctorId={doctor_id}&startDate={start_date}&endDate={end_date}'
        response = RequestHandler.get_api_response(request_path=request_path,
    ...
    @pytest.mark.security
    def test_security_doctor_should_not_get_ehr_appointment_data_with_recently_blocked_provider_token_cache_false(self):
    ...
    def test_doctor_should_not_get_ehr_appointment_data_for_invalid_date_format(self):
        ...
        request_path = f'lynx/appointments?cache.invalidateCache=false&doctorId={doctor_id}&startDate={start_date}&endDate={invalid_formatted_end_date}'
        response = RequestHandler.get_api_response(user_name=pytest.configs.get_config('ehr_lynx_enabled_rt_provider'),
                                                   password=pytest.configs.get_config('all_provider_password'),
                                                   request_path=request_path)
        with allure.step('Proper massage, status_code and reason should be returned'):
    ...
    def test_doctor_should_not_get_ehr_appointment_data_for_blocked_doctor_id(self):
        ...
        request_path = f'lynx/appointments?cache.invalidateCache=false&doctorId={blocked_doctor_id}&startDate={start_date}&endDate={end_date}'
        response = RequestHandler.get_api_response(user_name=pytest.configs.get_config('ehr_lynx_enabled_rt_provider1'),
                                                   password=pytest.configs.get_config('all_provider_password'),
                                                   request_path=request_path)
        with allure.step('Proper massage, status_code and reason should be returned'):
    ...
    # @allure.severity(allure.severity_level.NORMAL)
    # @pytest.mark.security
    # def test_doctor_should_not_get_ehr_appointment_data_with_token_from_another_doctor(self):
    #     email = pytest.configs.get_config('ehr_lynx_enabled_rt_provider')
    #     password = pytest.configs.get_config('all_provider_password')
    #     doctor_id = pytest.configs.get_config('ehr_lynx_enabled_rt_provider_id')
    #     another_doctor_auth_token = RequestHandler.get_auth_token(user_name=pytest.configs.get_config("lynx_enabled_rt_provider"),
    #                                                               password=pytest.configs.get_config("all_provider_password"))
    #     request_path = f'lynx/appointments?cache.invalidateCache=false&doctorId={doctor_id}&startDate={start_date}&endDate={end_date}'
    #     response = RequestHandler.get_api_response(request_path=request_path, user_name=email, password=password, token=another_doctor_auth_token)
    #     json_response = response.json()
    #     with allure.step('Proper massage, status_code and reason should be returned'):
    #         assert response.status_code == 500
    #         assert response.reason == 'Internal Server Error'
    ...
    #         assert json_response['status'] == 500
    #         assert json_response['error'] == 'Internal Server Error'
    #         assert json_response['message'] == 'The DoctorId 888889265 is not integrated with any Provider.'
    #         assert json_response['path'] == f'/lynx/appointments'

    # @allure.severity(allure.severity_level.CRITICAL)
    # @pytest.mark.security
    # def test_security_doctor_should_not_get_ehr_appointment_data_with_lynxenabled_provider_with_a_token_from_a_lynxdisabled_provider(self):
    #     email = pytest.configs.get_config('ehr_enabled_nrt_provider')
    #     password = pytest.configs.get_config('all_provider_password')
    #     doctor_id = pytest.configs.get_config('lynx_enabled_doctor1_id')
    #     lynx_disabled_rt_doctor_auth_token = RequestHandler.get_auth_token(user_name=email, password=password)
    #     request_path = f'lynx/appointments?cache.invalidateCache=false&doctorId={doctor_id}&startDate={start_date}&endDate={end_date}'
    #     response = RequestHandler.get_api_response(request_path=request_path, token=lynx_disabled_rt_doctor_auth_token)
    #     json_response = response.json()
    #     with allure.step('Proper massage, status_code and reason should be returned'):
    #         assert response.status_code == 500
    #         assert response.reason == 'Internal Server Error'
    ...
    #         assert json_response['status'] == 500
    #         assert json_response['error'] == 'Internal Server Error'
    #         assert 'could not be parsed at index 0' in json_response['message']
    #         assert json_response['path'] == f'/lynx/appointments'

    @allure.severity(allure.severity_level.CRITICAL)
    @pytest.mark.sanity
    def test_get_ehr_patient_meta_data_with_cache_false(self):
        doctor_id = pytest.configs.get_config('ehr_lynx_enabled_rt_provider_id')
        request_path = f'lynx/appointments?cache.invalidateCache=false&startDate={start_date}&endDate={end_date}&doctorId={doctor_id}'
        response = RequestHandler.get_api_response(user_name=pytest.configs.get_config('ehr_lynx_enabled_rt_provider'),
                                                   password=pytest.configs.get_config('all_provider_password'),
                                                   request_path=request_path)
        ...
        request_path = f'lynx/patient-metadata/{uuid}?noteId=111111&cache.invalidateCache=false'
        response = RequestHandler.get_api_response(user_name=pytest.configs.get_config('ehr_lynx_enabled_rt_provider'),
                                                   password=pytest.configs.get_config('all_provider_password'),
                                                   request_path=request_path)
        with allure.step('Proper massage, status_code and reason should be returned'):
            ...
            assert json_response['message'] == 'success'
            assert json_response['code'] == '000'
            assert json_response['data']['noteId'] == '111111'
    ...
    def test_security_doctor_should_not_get_ehr_patient_meta_data_with_algorithm_set_as_none_token(self):
        doctor_id = pytest.configs.get_config('ehr_lynx_enabled_rt_provider_id')
        request_path = f'lynx/appointments?cache.invalidateCache=false&startDate={start_date}&doctorId={doctor_id}'
        response = RequestHandler.get_api_response(user_name=pytest.configs.get_config('ehr_lynx_enabled_rt_provider'),
                                                   password=pytest.configs.get_config('all_provider_password'),
                                                   request_path=request_path)
    ...
    @allure.severity(allure.severity_level.NORMAL)
    @pytest.mark.negative
    def test_post_method_should_not_support_for_get_ehr_patient_meta_data_with_cache_false_endpoint(self):
        doctor_id = pytest.configs.get_config('ehr_lynx_enabled_rt_provider_id')
        request_path = f'lynx/appointments?cache.invalidateCache=false&startDate={start_date}&doctorId={doctor_id}'
        response = RequestHandler.get_api_response(user_name=pytest.configs.get_config('ehr_lynx_enabled_rt_provider'),
                                                   password=pytest.configs.get_config('all_provider_password'),
    ...
    @allure.severity(allure.severity_level.NORMAL)
    @pytest.mark.negative
    def test_put_method_should_not_support_for_get_ehr_patient_meta_data_with_cache_false_endpoint(self):
        doctor_id = pytest.configs.get_config('ehr_lynx_enabled_rt_provider_id')
        request_path = f'lynx/appointments?cache.invalidateCache=false&startDate={start_date}&doctorId={doctor_id}'
        response = RequestHandler.get_api_response(user_name=pytest.configs.get_config('ehr_lynx_enabled_rt_provider'),
                                                   password=pytest.configs.get_config('all_provider_password'),
    ...
    @allure.severity(allure.severity_level.NORMAL)
    @pytest.mark.negative
    def test_patch_method_should_not_support_for_get_ehr_patient_meta_data_with_cache_false_endpoint(self):
        doctor_id = pytest.configs.get_config('ehr_lynx_enabled_rt_provider_id')
        request_path = f'lynx/appointments?cache.invalidateCache=false&startDate={start_date}&doctorId={doctor_id}'
        response = RequestHandler.get_api_response(user_name=pytest.configs.get_config('ehr_lynx_enabled_rt_provider'),
                                                   password=pytest.configs.get_config('all_provider_password'),
    ...
    @allure.severity(allure.severity_level.NORMAL)
    @pytest.mark.negative
    def test_delete_method_should_not_support_for_get_ehr_patient_meta_data_with_cache_false_endpoint(self):
        doctor_id = pytest.configs.get_config('ehr_lynx_enabled_rt_provider_id')
        request_path = f'lynx/appointments?cache.invalidateCache=false&startDate={start_date}&doctorId={doctor_id}'
        response = RequestHandler.get_api_response(user_name=pytest.configs.get_config('ehr_lynx_enabled_rt_provider'),
                                                   password=pytest.configs.get_config('all_provider_password'),
    ...
    @pytest.mark.security
    def test_doctor_should_not_get_ehr_appointment_data_with_changed_password_previous_token_cache_false(self):
